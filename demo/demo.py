...
if __name__ == '__main__':
    ...
    # Input field for user ID with a custom CSS class
    ...
    if button:
        # Use the Google Scholar ID provided by the user
        scholar_id = User_id
        ...
        # Display the HTML with custom styling
        st.markdown("<div class='citation-map'>Citaion Map</div>", unsafe_allow_html=True)
        components.html(html_content, height=600)

    # Clear button logic
    if button_clear:
        st.session_state.user_id = ""  # Clear input field
